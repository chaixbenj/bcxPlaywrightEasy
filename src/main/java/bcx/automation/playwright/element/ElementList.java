...
/**
 * classe gérant les actions et assertions sur les list Select/Option UL/LI
 * @author bcx
 *
 */
        ...
        if (option!=null) {
            try {
                if (this.getTagName().equalsIgnoreCase("SELECT")) {
                    ...
                    if (!result.equals(Reporter.PASS_STATUS)) this.getTestContext().getReport().log(result, action + " >> " + option , getName(), null , null, null, null);
                }
    ...
    public void assertOptionsInList(String[] options, boolean inList) {
        ...
        for(String valueToAssert: options) {
            if (valueToAssert!=null) {
                ...
                this.getTestContext().getReport().log((inList?Reporter.FAIL_NEXT_STATUS:Reporter.PASS_STATUS), action + " " + inList + " >> " + Arrays.toString(options), getName(), null , null, null);
            }
        }
